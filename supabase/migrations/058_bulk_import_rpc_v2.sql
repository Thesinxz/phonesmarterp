-- ============================================================
-- Migration 058: Robust Bulk Import RPC V2
-- ============================================================

-- Recriamos a função para aceitar empresa_id explicitamente.
-- Isso evita erros quando o usuário tem múltiplos vínculos ou lentidão no JOIN de auth.
DROP FUNCTION IF EXISTS public.importar_produtos_massa(p_produtos JSONB);
DROP FUNCTION IF EXISTS public.importar_produtos_massa(p_produtos JSONB, p_empresa_id UUID);
...
BEGIN
  ...
  -- Como é SECURITY DEFINER, garantimos que o p_empresa_id informado 
  -- realmente pertence ao usuário que está chamando a função.
  IF NOT EXISTS (
