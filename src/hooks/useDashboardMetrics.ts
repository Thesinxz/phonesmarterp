    ...
    useEffect(() => {
        const fetchMetrics = async () => {
            try {
                ...
                // Se não houver status_pedido, assume que é venda PDV (finalizada)
                // O dashboard mostraPedidos Pendentes para vendas tipo "pedido"
        ...
        const channel = supabase
            .channel(`dashboard-metrics-${empresa_id}`)
            .on(
                "postgres_changes",
                { event: "*", schema: "public", table: "ordens_servico", filter: `empresa_id=eq.${empresa_id}` },
                () => fetchMetrics()
            )
            .on(
                "postgres_changes",
            ...
            .on(
                "postgres_changes",
                { event: "*", schema: "public", table: "financeiro", filter: `empresa_id=eq.${empresa_id}` },
                () => fetchMetrics()
            )
            .on(
                "postgres_changes",
                { event: "*", schema: "public", table: "vendas", filter: `empresa_id=eq.${empresa_id}` },
                () => fetchMetrics()
            )
            .subscribe((status) => {
