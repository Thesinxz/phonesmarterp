...
export function PagarListaClient({ initialTitulos, initialResumo, empresaId }: Props) {
    ...
    useEffect(() => {
        // Only trigger loadData if it's not the initial mount or if filters changed
        if (statusFilter === "todos" && !filterStart && !filterEnd && !searchTerm) return;
        loadData();
    }, [statusFilter, filterStart, filterEnd]);
    ...
    async function loadData(background = false) {
        ...
        try {
            const [resumoData, titulosData] = await Promise.all([
                getResumoTitulos(empresaId, 'pagar', { startDate: filterStart, endDate: filterEnd }),
                getTitulos(empresaId, 'pagar', 1, 50, {
                    status: statusFilter !== "todos" ? [statusFilter as any] : undefined,
                    busca: searchTerm,
                    data_inicio: filterStart,
                    data_fim: filterEnd
                })
            ]);
    ...
    return (
        <FeatureGate
            feature="contas_pagar_receber"
            featureName="Contas a Pagar"
            description="Controle suas despesas, fornecedores e fluxo de caixa de saída com precisão."
        >
            <div className="space-y-6 page-enter pb-10">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-slate-500 text-sm mt-0.5">Gestão de contas, fornecedores e obrigações ({titulos.length})</p>
                    </div>
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    ...
                                                    {item.valor_pago_centavos > 0 && item.status !== 'pago' && (
                                                        <p className="text-[10px] text-slate-400">
                                                            Falta: {formatCurrency(item.valor_total_centavos - (item.valor_pago_centavos || 0))}
                                                        </p>
                                                    )}
                                                </td>
